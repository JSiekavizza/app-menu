...
const PizzaProducts = () => {
  ...
  const pizzaCategoria = data.categorias?.find(
    (categoria) => categoria.nombre === "Pizza"
  );

  if (!pizzaCategoria) {
    return <div>No se encontraron productos para Pizza.</div>;
  }
    ...
    <>
      ...
      <div className="bg-yellow-500  p-2 mt-5">
        <div className="flex justify-center ">
          <h2 className="text-3xl font-extrabold">Productos de Pizza</h2>
        </div>
        <div className=" grid grid-cols-1 lg:grid-cols-3 gap-2 w-full p-3">
          {pizzaCategoria.productos.map((producto) => (
